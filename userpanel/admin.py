...
class MemberAdmin(admin.ModelAdmin):
    list_display = ('name', 'userid', 'email', 'phone' ,'username','join_date', 'sponsor','type', )
    # exclude = ('Action',)
    list_per_page = 100
...
class Adminnewlotteriesthreem(admin.ModelAdmin):
    list_display = ('id','created_at', 'status', 'total_amount', 'min_number','Max_number', 'Result', 'total_winning' , 'all_amount')
    list_per_page = 10
    search_fields = ('id',)
    ...
    def total_winning(self, obj):
        sum = BetSingle.objects.filter(period=obj.id, timing=3, status = 'Win').aggregate(Sum('winnningamount'))
        return sum['winnningamount__sum']
...
class Adminnewlotteries15m(admin.ModelAdmin):
    ...
    def total_winning(self, obj):
        sum = BetSingle.objects.filter(period=obj.id, timing=15, status = 'Win').aggregate(Sum('winnningamount'))
        return sum['winnningamount__sum']
...
class Adminnewlotteriesnine(admin.ModelAdmin):
    list_display = ('id','created_at', 'status', 'total_amount', 'min_number','Max_number', 'Result', 'total_winning')
    list_per_page = 10
    search_fields = ('id',)
    list_display_links = None
    ...
    def total_winning(self, obj):
        sum = BetSingle.objects.filter(period=obj.id, timing=10, status = 'Win').aggregate(Sum('winnningamount'))
        return sum['winnningamount__sum']
